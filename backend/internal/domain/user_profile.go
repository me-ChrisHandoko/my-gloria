...
// UserProfile represents a user profile with custom JWT authentication and employee data
type UserProfile struct {
	...
	// Relations (constraint:- disables FK creation to avoid circular dependency during migration)
	DataKaryawan           *DataKaryawan      `json:"data_karyawan,omitempty" gorm:"foreignKey:NIP;references:NIP;constraint:-"`
	ApiKeys                []ApiKey           `json:"-" gorm:"foreignKey:UserID"`
	RefreshTokens          []RefreshToken     `json:"-" gorm:"foreignKey:UserProfileID"`
