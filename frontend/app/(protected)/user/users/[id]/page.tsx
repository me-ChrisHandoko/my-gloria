...
import { ArrowLeft, Edit, Trash2, Mail, UserCheck, UserX, Shield, Briefcase, X } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            onClick={() => router.push(`/user/users/${id}/edit`)}
          >
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <AssignRoleDialog
                  userId={id}
                  assignedRoleIds={userRoles?.map(r => r.role_id) || []}
                />
              </div>
              {isLoadingRoles ? (
                  ...
                  {userRoles.map((roleAssignment) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        {roleAssignment.role?.hierarchy_level && (
                          <span className="text-xs text-muted-foreground">
                            Level {roleAssignment.role.hierarchy_level}
                          </span>
                        )}
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => openRevokeRoleDialog(
                          roleAssignment.id,
                          roleAssignment.role?.name || "Unknown Role"
                        )}
                        disabled={revokingRoleId === roleAssignment.id}
                      >
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <AssignPositionDialog
                  userId={id}
                  assignedPositionIds={userPositions?.map(p => p.position_id) || []}
                />
              </div>
              {isLoadingPositions ? (
                  ...
                  {userPositions.map((positionAssignment) => (
                    <div key={positionAssignment.id} className="flex items-center justify-between p-2 border rounded-lg">
                      <div className="space-y-1 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="font-medium text-sm">
                            {positionAssignment.position?.name || "Unknown Position"}
                          </span>
                          {positionAssignment.is_plt && (
                            <Badge variant="warning" className="text-xs py-0">PLT</Badge>
                          )}
                          {!positionAssignment.is_active && (
                            <Badge variant="secondary" className="text-xs py-0">Tidak Aktif</Badge>
                          )}
                        </div>
                        <div className="text-xs text-muted-foreground space-y-1">
                          {positionAssignment.position?.department?.name && (
                            <div>Departemen: {positionAssignment.position.department.name}</div>
                          )}
                          {positionAssignment.sk_number && (
                            <div>SK: {positionAssignment.sk_number}</div>
                          )}
                          <div className="flex gap-3">
                            <span>
                              Mulai: {new Date(positionAssignment.start_date).toLocaleDateString('id-ID')}
                            </span>
                            {positionAssignment.end_date && (
                              <span>
                                Berakhir: {new Date(positionAssignment.end_date).toLocaleDateString('id-ID')}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => openRevokePositionDialog(
                          positionAssignment.id,
                          positionAssignment.position?.name || "Unknown Position"
                        )}
                        disabled={revokingPositionId === positionAssignment.id}
                      >
                        <X className="h-4 w-4 text-destructive" />
                      </Button>
                    </div>
      ...
      <AlertDialog open={revokeRoleDialogOpen} onOpenChange={setRevokeRoleDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Apakah Anda yakin ingin me-revoke role{" "}
              <strong>{pendingRoleRevoke?.name}</strong> dari user ini?
              <br />
              <br />
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setPendingRoleRevoke(null)}>
              Batal
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={handleConfirmRevokeRole}
      ...
      <AlertDialog open={revokePositionDialogOpen} onOpenChange={setRevokePositionDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Apakah Anda yakin ingin me-revoke posisi{" "}
              <strong>{pendingPositionRevoke?.name}</strong> dari user ini?
              <br />
              <br />
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setPendingPositionRevoke(null)}>
              Batal
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={handleConfirmRevokePosition}
              disabled={revokingPositionId !== null}
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
            >
              {revokingPositionId ? (
                <>
