...
import { useGetWorkflowRuleByIdQuery, useUpdateWorkflowRuleMutation, useGetPositionsQuery, useGetSchoolsQuery } from "@/lib/store/services/organizationApi";
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-muted-foreground">{getWorkflowTypeLabel(rule.workflow_type)}</p>
            </div>
                ...
                <Card>
                    ...
                    <CardContent className="grid gap-4 md:grid-cols-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-sm text-muted-foreground">Pilih sekolah untuk aturan khusus, atau Global untuk semua sekolah</p>
                        </div>

                        <div className="space-y-2">
                            <Label htmlFor="priority">Prioritas</Label>
                            <Input id="priority" type="number" min={1} max={100} placeholder="1-100" {...register("priority", { valueAsNumber: true })} />
                            <p className="text-sm text-muted-foreground">Urutan prioritas (1 = tertinggi)</p>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-sm text-muted-foreground">Posisi yang membuat/mengajukan workflow</p>
                        </div>

                        <div className="space-y-2">
                            <Label>Status</Label>
                            <div className="flex items-center gap-2 pt-2">
                                <input id="is_active" type="checkbox" className="h-4 w-4 rounded border-gray-300" {...register("is_active")} />
                        ...
                        <div className="space-y-2 md:col-span-2">
                            ...
                            <Textarea id="description" placeholder="Deskripsi singkat aturan workflow" rows={2} {...register("description")} />
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>Tentukan urutan penyetuju workflow (multi-level approval)</CardDescription>
                            </div>
                                ...
                                <div key={field.id} className="p-4 border rounded-lg space-y-4 bg-muted/30">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            ...
                                            {watch(`steps.${index}.is_optional`) && <Badge variant="outline">Opsional</Badge>}
                                        </div>
                                        <div className="flex items-center gap-1">
                                            <Button type="button" variant="ghost" size="sm" onClick={() => moveStepUp(index)} disabled={index === 0}>
                                                <ArrowUp className="h-4 w-4" />
                                            </Button>
                                            <Button type="button" variant="ghost" size="sm" onClick={() => moveStepDown(index)} disabled={index === fields.length - 1}>
                                                <ArrowDown className="h-4 w-4" />
                                            </Button>
                                            <Button type="button" variant="ghost" size="sm" onClick={() => removeStep(index)} className="text-destructive hover:text-destructive">
                                    ...
                                    <div className="grid gap-4 md:grid-cols-3">
                                        <div className="space-y-2">
                                            ...
                                            <Select
                                                ...
                                                disabled={isLoadingPositions}
                                            >
                                                <SelectTrigger className={errors.steps?.[index]?.approver_position_id ? "border-destructive" : ""}>
                                            ...
                                            {errors.steps?.[index]?.approver_position_id && <p className="text-sm text-destructive">{errors.steps[index]?.approver_position_id?.message}</p>}
                                        </div>

                                        <div className="space-y-2">
                                            <Label>Nama Langkah</Label>
                                            <Input placeholder="Contoh: Persetujuan Manager" {...register(`steps.${index}.step_name`)} />
                                        </div>

                                        <div className="space-y-2">
                                            <Label>Tipe</Label>
                                            <div className="flex items-center gap-2 pt-2">
                                                <input type="checkbox" id={`steps.${index}.is_optional`} className="h-4 w-4 rounded border-gray-300" {...register(`steps.${index}.is_optional`)} />
                                                <Label htmlFor={`steps.${index}.is_optional`} className="cursor-pointer font-normal">
                                                    Langkah Opsional
                                                </Label>
                                            </div>
                                            <p className="text-xs text-muted-foreground">Centang jika langkah ini bisa dilewati</p>
                                        </div>
                                    </div>
                                </div>
                            ))
