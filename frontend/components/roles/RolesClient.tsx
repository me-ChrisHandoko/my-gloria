 ...
 *
 * Client-side interactive component for role management.
...
import type { PaginatedRolesResponse, RoleFilter } from "@/lib/types/role";

interface RolesClientProps {
    initialData: PaginatedRolesResponse;
}
...
export default function RolesClient({ initialData }: RolesClientProps) {
    ...
    // Fetch roles with filters - use useMemo to prevent unnecessary re-renders
    const queryParams = useMemo<RoleFilter>(() => {
        ...
        if (typeFilter !== undefined) params.is_system_role = typeFilter;

        return params;
    }, [filters, debouncedSearch, statusFilter, typeFilter]);

    const { data: rolesData, isLoading, error, refetch } = useGetRolesQuery(queryParams);

    // Use initialData as fallback only for first render before query completes
    const displayData = rolesData || initialData;
    ...
    const handleSortChange = (sortBy: string) => {
        setFilters((prev) => {
            ...
            return {
                ...prev,
                sort_by: sortBy as RoleFilter["sort_by"],
                sort_order: "asc",
            };
        });
    };
            ...
            <Card>
                ...
                <div className="px-4 pt-4 pb-2">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                        ...
                        <div className="relative flex-1">
                            ...
                            <Input placeholder="Cari berdasarkan nama atau kode role..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
                        </div>
                        ...
                        {/* Type Filter */}
                        <Select value={typeFilter === undefined ? "all" : typeFilter ? "system" : "custom"} onValueChange={handleTypeFilterChange}>
                            <SelectTrigger className="w-full sm:w-[150px]">
                                <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                ...
                {isLoading && !displayData && (
                    <div className="py-12 flex flex-col items-center gap-4">
                        <LoadingSpinner />
                        <p className="text-sm text-muted-foreground">Memuat data role...</p>
                    </div>
                )}
                        ...
                        {displayData.data.length === 0 && !search && statusFilter === undefined && typeFilter === undefined ? (
                            <div className="px-4 pb-4">
                                <EmptyState
                                    ...
                                    description={
                                        <div className="space-y-2">
                                            <p>Sistem belum memiliki data role. Mulai dengan menambahkan role pertama untuk sistem YPK Gloria.</p>
                                            <p className="text-xs">Role yang ditambahkan akan menentukan hak akses pengguna dalam sistem dan dapat dikelola sesuai kebutuhan organisasi.</p>
                                        </div>
                                    }
                                    primaryAction={{
                                        label: "Tambah Role Pertama",
                                        onClick: () => {
                                            // Trigger CreateRoleButton
                                            const createBtn = document.querySelector("[data-create-role-btn]") as HTMLButtonElement;
                            ...
                            <div className="px-4 pb-4">
                                <EmptyState
                                    ...
                                    description={
                                        <div className="space-y-2">
                                            <p>Tidak ada role yang sesuai dengan kriteria pencarian atau filter Anda.</p>
                                            <div className="text-xs space-y-1 mt-3 text-left bg-muted/50 p-3 rounded-md">
                                                ...
                                                <ul className="list-disc list-inside space-y-1">
                                                    ...
                                                    <li>Coba ubah atau reset filter status dan tipe</li>
                                                    <li>Pastikan data role sudah ditambahkan ke sistem</li>
                                                </ul>
                                            </div>
                                        </div>
                                    }
                            ...
                            <>
                                ...
                                <div className="px-4 pt-2 pb-2">
                                    <RolesDataTable roles={displayData.data} sortBy={filters.sort_by} sortOrder={filters.sort_order} onSortChange={handleSortChange} />
                                </div>
                                ...
                                <div className="px-4 pb-4">
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                        ...
                                        <div className="text-sm text-muted-foreground text-center sm:text-left">
                                            Menampilkan {(displayData.page - 1) * displayData.page_size + 1}-{Math.min(displayData.page * displayData.page_size, displayData.total)} dari {displayData.total} role
                                        </div>
