 ...
 *
 * Client-side interactive component for permission management.
...
import type { PaginatedPermissionsResponse, PermissionFilter } from "@/lib/types/permission";
...
export default function PermissionsClient({ initialData }: PermissionsClientProps) {
    ...
    // Fetch permissions with filters - use useMemo to prevent unnecessary re-renders
    const queryParams = useMemo<PermissionFilter>(() => {
        const params: PermissionFilter = {
            ...filters,
        };

        if (debouncedSearch) params.search = debouncedSearch;
        if (statusFilter !== undefined) params.is_active = statusFilter;
        if (typeFilter !== undefined) params.is_system_permission = typeFilter;

        return params;
    }, [filters, debouncedSearch, statusFilter, typeFilter]);

    const { data: permissionsData, isLoading, error, refetch } = useGetPermissionsQuery(queryParams);

    // Use initialData as fallback only for first render before query completes
    const displayData = permissionsData || initialData;
    ...
    const handleSortChange = (sortBy: string) => {
        setFilters((prev) => {
            ...
            return {
                ...prev,
                sort_by: sortBy as PermissionFilter["sort_by"],
                sort_order: "asc",
            };
        });
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">Kelola permissions dan hak akses dalam sistem</p>
                </div>
                ...
                <div className="px-4 pt-4 pb-2">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                        ...
                        <div className="relative flex-1">
                            ...
                            <Input placeholder="Cari berdasarkan nama atau kode permission..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
                        </div>
                        ...
                        {displayData.data.length === 0 && !search && statusFilter === undefined && typeFilter === undefined ? (
                            <div className="px-4 pb-4">
                                <EmptyState
                                    ...
                                    description={
                                        <div className="space-y-2">
                                            <p>Sistem belum memiliki data permission. Mulai dengan menambahkan permission pertama untuk sistem kontrol akses.</p>
                                            <p className="text-xs">Permission yang ditambahkan akan digunakan untuk mengatur hak akses pengguna dalam sistem.</p>
                                        </div>
                                    }
                                    primaryAction={{
                                        label: "Tambah Permission Pertama",
                                        onClick: () => {
                                            // Trigger CreatePermissionButton
                            ...
                            <div className="px-4 pb-4">
                                <EmptyState
                                    ...
                                    description={
                                        <div className="space-y-2">
                                            <p>Tidak ada permission yang sesuai dengan kriteria pencarian atau filter Anda.</p>
                                            <div className="text-xs space-y-1 mt-3 text-left bg-muted/50 p-3 rounded-md">
                                                ...
                                                <ul className="list-disc list-inside space-y-1">
                                                    ...
                                                    <li>Coba ubah atau reset filter status/tipe</li>
                                                    <li>Pastikan data permission sudah ditambahkan ke sistem</li>
                                                </ul>
                                            </div>
                                        </div>
                                    }
                                ...
                                <div className="px-4 pb-4">
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                        ...
                                        <div className="text-sm text-muted-foreground text-center sm:text-left">
                                            Menampilkan {(displayData.page - 1) * displayData.page_size + 1}-{Math.min(displayData.page * displayData.page_size, displayData.total)} dari {displayData.total} permissions
                                        </div>
